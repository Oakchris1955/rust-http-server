...
pub fn read_line(mut connection: &mut Connection) -> io::Result<String> {
    ...
    loop {
        // Read 1 byte from the TCPStream
        let bytes_read = read_bytes(&mut connection, 1)?;
        let char_read = bytes_read[0] as char;
        // If found CRLF (\r\n), return string (without CRLF)
        if char_read == '\n' {
            if temp_string.chars().last().unwrap_or_else(|| unreachable!()) == '\r' {
                temp_string.pop();
                break;
            }
        }
...
pub fn read_bytes(connection: &mut Connection, bytes_to_read: usize) -> io::Result<Vec<u8>> {
    ...
    loop {
        ...
        let bytes_read = connection.stream.read(&mut temp_array)?;
        if bytes_read > 0 {
        ...
        if connection.inactive_since.elapsed().unwrap_or_default() > connection.timeout {
